...
def detect_chim(my_seqs = None, kmer_df = None, threads = None, columns = None):
    """
    ...
    if not columns or columns == "all": columns = kmer_df.columns 
    chim_per_col = {k:[] for k in columns}
    for col in columns:
        chims = []
        with Pool(threads) as pool:
            para_input = [(kmer_df[col],seq) for seq in my_seqs]
            for i, result in enumerate(pool.imap_unordered(process_chunk, para_input, chunksize=10)):
                if i % 500 == 0: print(f"{i}/{len(my_seqs)}/{str(col)}       ", end="\r")
                if result:chims.append(result)
        chim_per_col[col] = chims
    print("Execution time:",time.time() - start," "*20)
    return chim_per_col