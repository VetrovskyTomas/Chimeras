...
def process_chunk(para_input):
    chunk_dict, k_size = para_input
    #res_dic = {k:[] for k in chunk_dict.keys()}
    for k,v in chunk_dict.items():
        kmers = list(chain.from_iterable([extract_kmers(seq,k_size) for seq in v]))
        with gzip.open(f'/home/mg/ChimeraProject/pickles/all{k_size}mers/{k}.pickle.gzip', 'wb') as f:
...
def get_kmers_para_pickle(ref_seqs,k_size=50,threads = 1,name_patter = "(f__).+?(;)"): # dont need par_of_fasta_id
    """
    ...
    para_input = [({k:dic_seqs[k] for k in k_c},k_size) for k_c in key_chunks]
