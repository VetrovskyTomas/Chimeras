...
def process_file(args):
    ...
    print("len_kmers",len(kmers),kmers)
    ...
    for tr,tr_c in zip(thres,thres_counts):
        com_all_kmers = set([kmer for kmer, count in all_counts if count > tr_c]) # nechci set ale chci pozice - možná použít enumerate funkci ???
...
def uniq_kmers_with_positions(ref_seqs, out_dir, name_patter, k_vals = [], thres = [],n_cores=6):
    """
    Returns table with kmers for earch taxonomic item ( rows ) and abundance of the Kmer ( 10mer, 25mer etc. , 95%, 80% etc. ) ( columns) 
    Also retruns another table that contains only unique kmers in the column
    It shoul also return positions of the kmers
    """
    start = time.time()
